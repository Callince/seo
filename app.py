from flask import Flask, after_this_request, render_template, request, redirect, url_for, send_file, flash, session, jsonify, make_response
...
from utils.text_extractor import extract_text, correct_text, process_keywords, extract_keywords_tfidf, extract_keywords_rake, extract_keywords_combined
...
# Gmail API DISABLED - Using SMTP only with support@ and payment@ emails
# Removed Gmail API imports as per user request to use SMTP only
...
# Track crawling progress - REDIS-BASED SHARED STATE
from crawl_status_manager import CrawlStatusManager, CrawlStatusDict

# Initialize Redis-based crawl status manager
crawl_status_manager = CrawlStatusManager(
    redis_host=os.environ.get('REDIS_HOST', 'localhost'),
    redis_port=int(os.environ.get('REDIS_PORT', 6379)),
    redis_db=1,  # Use db=1 for crawl status (db=0 is for cache)
...
app.logger.info("✅ Crawl status manager initialized with Redis backend")
...
def update_database_tables():
    ...
    This function checks for missing columns and adds them if they don't exist.
    SQLAlchemy's db.create_all() only creates new tables, not new columns in existing tables.
    """
    from sqlalchemy import inspect, text

    inspector = inspect(db.engine)

    # Define the columns to add for each table
    ...
    # Special migration: Make subscription_id nullable in usage_logs
    if 'usage_logs' in inspector.get_table_names():
        try:
            # PostgreSQL syntax to drop NOT NULL constraint
            alter_sql = text("ALTER TABLE usage_logs ALTER COLUMN subscription_id DROP NOT NULL")
            db.session.execute(alter_sql)
            db.session.commit()
            app.logger.info("Made subscription_id nullable in usage_logs table")
        except Exception as e:
            db.session.rollback()
            # This might fail if already nullable, which is fine
            app.logger.debug(f"subscription_id nullable migration: {str(e)}")

    for table_name, columns in table_updates.items():
        # Check if table exists
        if table_name not in inspector.get_table_names():
            app.logger.info(f"Table {table_name} does not exist yet, will be created by db.create_all()")
            continue
        ...
        for column_name, column_definition in columns:
            if column_name not in existing_columns:
                try:
                    ...
                    db.session.rollback()
                    app.logger.error(f"Error adding column '{column_name}' to table '{table_name}': {str(e)}")
            else:
                app.logger.debug(f"Column '{column_name}' already exists in table '{table_name}'")
...
def create_app():
    ...
    with app.app_context():
        try:
            # Update existing tables with new columns first
            update_database_tables()

            # Create tables (for any new tables that don't exist)
            db.create_all()
...
# Initialize Flask-Session for better session management
...
CSRF_EXEMPT_ROUTES = [
    ...
    'admin_add_blog_category',
    'admin_edit_blog_category',
    'admin_delete_blog_category',
    'admin_blogs',
    'admin_blog_upload_image',
    'admin_add_blog',
    'admin_edit_blog',
    'admin_delete_blog',
    'public_blogs',
    ...
    # WebStory Routes
    'admin_webstories',
    'admin_webstory_upload_image',
    'admin_webstory_check_slug',
    'admin_add_webstory',
    'admin_edit_webstory',
    'admin_delete_webstory',
    'public_webstories',
    'webstory_detail',
]
@app.before_request
...
def handle_internal_error(e):
    """
    Handle internal server errors - return JSON for AJAX/API requests
    """
    ...
    # Check if it's an AJAX/API request (fetch, XMLHttpRequest, or JSON accept header)
    is_ajax = (
        request.headers.get('X-Requested-With') == 'XMLHttpRequest' or
        request.headers.get('Accept', '').startswith('application/json') or
        request.content_type == 'application/json' or
        request.endpoint and 'upload' in request.endpoint
    )
...
def handle_exception(e):
    """
    Handle all unhandled exceptions - return JSON for AJAX/API requests
    """
...
def add_no_cache_headers(response):
    """Add no-cache headers to prevent browser caching of dynamic pages"""
    if request.endpoint in ['imagesearch', 'urlsearch', 'hsearch', 'keyword_search']:
...
def clear_cache():
    cache.clear()
    flash('Application cache cleared!', 'success')
...
# Database session cleanup handlers to prevent stale connections
...
# NOTE: create_app() is called at the end of the file after all models are defined
# This ensures db.create_all() picks up all model columns including schema_data
...
class User(db.Model, UserMixin):
    ...
    # Trial tokens for new users (only usable without subscription)
...
class SubscribedUser(db.Model):
    ...
    def get_total_usage_limit(self):
        """
        Get the total usage limit (subscription limit + custom additional limit if any)
        """
...
def remove_search_history(user_id, search_query):
    ...
    try:
        ...
        db.session.commit()
        if deleted > 0:
            app.logger.info(f"Removed {deleted} search history entries for user {user_id}, URL: {search_query}")
        return True

    except SQLAlchemyError as e:
        print(f"Error removing search history: {e}")
        db.session.rollback()
        return False
...
class UsageLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    subscription_id = db.Column(db.Integer, db.ForeignKey('subscribed_users.id'), nullable=True)  # Nullable for trial users
    ...
    is_trial = db.Column(db.Boolean, default=False)  # Flag to identify trial usage
...
class Admin(db.Model):
    ...
    def admin_permissions(self, required_permission):
        ...
        try:
            ...
            # Not in request context, skip to stored permissions check
            pass
    ...
    def check_permission(email_id, required_permission):
        ...
        if not admin:
            app.logger.error(f"check_permission: Admin not found for email {email_id}")
            return False

        app.logger.info(f"check_permission: Checking {required_permission} for {email_id}")
        app.logger.info(f"check_permission: Admin permissions = {admin.permission}")
        ...
        if request.method == 'POST':
            ...
            if form_email == email_id:
                ...
                app.logger.info(f"check_permission: POST check result = {result}")
                return result

        # Otherwise check stored permissions
        result = admin.admin_permissions(required_permission)
        app.logger.info(f"check_permission: admin_permissions() result = {result}")
        return result

    def __repr__(self):
        return f"<Admin {self.NAME} - {self.role}>"

# Add this new model to your app.py file (around line 600, after other models)
...
class EmailLog(db.Model):
    ...
    def log_email(recipient_email, recipient_name, email_type, subject, user_id=None, 
        ...
        try:
            ...
            app.logger.info(f"Email log created: {email_type} to {recipient_email} - {status}")
            return True
...
def create_super_admin():
    ...
    super_admin_email = "manikandan@fourdm.com"  # Lowercase for consistency
    existing_admin = Admin.query.filter_by(email_id=super_admin_email).first()

    if existing_admin:
        # Super admin already exists, do not update
        logging.info("Super admin already exists")
        return

    # Create super admin with all permissions including the new one
    # Note: permission column is PostgreSQL ARRAY type, pass a Python list directly
    super_admin = Admin(
        email_id=super_admin_email,
        NAME="Super Admin",
        role="Super Admin",
        phone_number="8132156825",  # Change this if needed
        assigned_by="System",
        permission=[
            "dashboard",
            "manage_roles",
...
class Blog(db.Model):
    ...
    publish_date = db.Column(db.Date, nullable=True)  # Publish date for the blog
...
class WebStory(db.Model):
    """Web Stories for visual storytelling content"""
...
def ensure_default_website_settings():
    ...
    try:
        ...
        db.session.commit()
        app.logger.info("Default website settings ensured")
        
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error ensuring default website settings: {str(e)}")
...
"""
        ...
        try:
            ...
            app.config['MAIL_USERNAME'] = app.config.get('MAIL_PAYMENT_USERNAME', sender_email)
            app.config['MAIL_PASSWORD'] = app.config.get('MAIL_PAYMENT_PASSWORD')
            mail.init_app(app)  # Reinitialize mail with new credentials
            ...
            mail.init_app(app)  # Reinitialize mail with support credentials
...
def highlight_keywords(text, keywords_colors):
    ...
    that highlights it with background color and bold font.
...
# Add these functions to your app.py file (around line 1300, after other email functions)
...
def send_subscription_expiry_warning_email(user, subscription, subscribed_user):
    ...
    try:
        app.logger.info(f"Starting to send expiry warning email to {user.company_email}")
        ...
        site_url = app.config.get('SITE_URL', 'https://seodada.com')
        logo_url = f"{site_url}/static/images/seodada.png"

        app.logger.info(f"Email subject: {subject}")
        app.logger.info(f"Sending from: {sender_email}")
        app.logger.info(f"Sending to: {user.company_email}")
...
"""
        ...
        app.logger.info(f"Sending email via SMTP from {sender_email}...")
        ...
        try:
            ...
            app.config['MAIL_PASSWORD'] = original_password
            mail.init_app(app)  # Reinitialize mail with support credentials
        app.logger.info(f"Email sent successfully via SMTP")
        ...
        app.logger.info(f"✓ Subscription expiry warning email sent successfully to {user.company_email}")
        return True

    except Exception as e:
        error_details = f"{type(e).__name__}: {str(e)}"
        app.logger.error(f"✗ Failed to send subscription expiry warning email to {user.company_email}")
        ...
        try:
            ...
            app.logger.error(f"Failed to log email error: {str(log_error)}")
        ...
        for subscribed_user, user, subscription in expiring_subscriptions:
            try:
                ...
                if existing_notification:
                    app.logger.info(f"Skipping notification to {user.company_email} - already sent today")
                    continue
                ...
                app.logger.info(f"Sent expiry notification to {user.company_email} for {subscription.plan} subscription expiring at {subscribed_user.end_date}")

            except Exception as e:
                app.logger.error(f"Failed to send expiry notification to {user.company_email}: {str(e)}")
                continue
        
        app.logger.info(f"Subscription expiry check completed. Sent {notifications_sent} notifications.")
        return notifications_sent
        
    except Exception as e:
        app.logger.error(f"Error in check_and_notify_expiring_subscriptions: {str(e)}")
        return 0


# Update the existing setup_daily_cleanup_scheduler function to include the expiry check
def setup_daily_cleanup_scheduler():
    """
    Set up daily cleanup scheduler using APScheduler (FIXED VERSION WITH APP CONTEXT)
    """
    try:
        ...
        # Wrapper function for cleanup with app context
        def run_cleanup_with_context():
            with app.app_context():
                cleanup_old_crawl_data(days_to_keep=7)

        # Add daily cleanup job at 2:00 AM UTC
        scheduler.add_job(
            func=run_cleanup_with_context,
            trigger="cron",
            hour=2,
            minute=0,
        ...
        # Wrapper function for memory cleanup with app context
        def run_memory_cleanup_with_context():
            with app.app_context():
                cleanup_crawl_status_memory()

        # Add hourly memory cleanup job
        scheduler.add_job(
            func=run_memory_cleanup_with_context,
            trigger="cron",
            minute=0,
        ...
        # Wrapper function for expiry check with app context - CRITICAL FIX
        def run_expiry_check_with_context():
            with app.app_context():
                try:
                    app.logger.info("Running scheduled subscription expiry check...")
                    notifications_sent = check_and_notify_expiring_subscriptions()
                    app.logger.info(f"Expiry check completed: {notifications_sent} notifications sent")
                except Exception as e:
                    app.logger.error(f"Error in scheduled expiry check: {str(e)}")
                    app.logger.error(traceback.format_exc())

        # ADD NEW JOB: Daily subscription expiry check at 9:00 AM UTC
        scheduler.add_job(
            func=run_expiry_check_with_context,
            trigger="cron",
            hour=9,  # 9 AM UTC
            minute=0,
        ...
        # Start the scheduler
        scheduler.start()
        app.logger.info("Daily scheduler started with all jobs:")
        app.logger.info("  - Daily file cleanup: 2:00 AM UTC (keeps files for 7 days)")
        app.logger.info("  - Hourly memory cleanup: every hour")
        app.logger.info("  - Daily subscription expiry check: 9:00 AM UTC")
...
# Add this admin route to manually test the expiry notification system
@app.route('/admin/test-expiry-notifications')
@admin_required
def test_expiry_notifications():
    ...
    try:
        ...
        app.logger.error(f"Error testing expiry notifications: {str(e)}")
        flash('Error sending expiry notifications. Check logs for details.', 'danger')
    
    return redirect(url_for('admin_subscriptions'))


# Add this route for cron job to trigger expiry checks (optional)
@app.route('/cron/check-expiring-subscriptions')
def cron_check_expiring_subscriptions():
    ...
    try:
        ...
        app.logger.info(f"Expiry check cron job completed: {result}")
        return jsonify(result)
        
    except Exception as e:
        app.logger.error(f"Expiry check cron job failed: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e),
            'timestamp': datetime.now(UTC).isoformat()
...
# Add test email route for debugging
@app.route('/admin/test-email-config')
@admin_required
...
"""
        ...
        <html>
            <body style="font-family: Arial, sans-serif; padding: 20px;">
                ...
                <ul>
                    <li><strong>MAIL_SERVER:</strong> {app.config.get('MAIL_SERVER')}</li>
                    <li><strong>MAIL_PORT:</strong> {app.config.get('MAIL_PORT')}</li>
                    <li><strong>MAIL_USE_TLS:</strong> {app.config.get('MAIL_USE_TLS')}</li>
                    <li><strong>MAIL_USE_SSL:</strong> {app.config.get('MAIL_USE_SSL')}</li>
                    <li><strong>MAIL_USERNAME:</strong> {app.config.get('MAIL_USERNAME')}</li>
                    <li><strong>Sender:</strong> {app.config.get('MAIL_USERNAME')}</li>
        ...
        app.logger.info(f"Sending test email to {email_id}")
        mail.send(message)
        app.logger.info(f"Test email sent successfully to {email_id}")

        flash(f'Test email sent successfully to {email_id}. Please check your inbox.', 'success')

    except Exception as e:
        error_msg = f"{type(e).__name__}: {str(e)}"
        app.logger.error(f"Failed to send test email: {error_msg}")
        app.logger.error(f"Traceback: {traceback.format_exc()}")
...
def admin_login():
    # Clear any existing flash messages on GET request (fresh page load)
...
def admin_search_history():
    ...
    # Total searches = sum of all users' tokens used from UsageLog
    total_searches = 0
    ...
    for log in all_logs:
        try:
            ...
            app.logger.error(f"Error parsing token usage from log {log.id}: {str(e)}")
            total_searches += 1
    ...
    # Today's searches = sum of tokens used today from UsageLog
    ...
    for log in today_logs:
        try:
            ...
            app.logger.error(f"Error parsing today's token usage from log {log.id}: {str(e)}")
            searches_today += 1
...
def admin_payments():
    ...
    # stats['average_payment_value'] = (
    #     stats['total_revenue'] / stats['completed_payments'] 
...
@app.route('/admin/contact_submissions/<int:id>/send_reply', methods=['POST'])
def send_reply(id):
...
def update_contact_submission(submission_id):
    try:
        ...
        # Check if it's an AJAX request (only check for XMLHttpRequest header)
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return jsonify({
                'success': True,
...
@app.route('/admin/email_logs/<int:log_id>')
@admin_required
...
@app.route('/admin/email_logs/export')
@admin_required
...
@app.route('/admin/email_logs/retry/<int:log_id>', methods=['POST'])
@admin_required
def admin_retry_email(log_id):
    ...
    if email_log.status != 'failed':
        ...
        return redirect(url_for('admin_email_log_detail', log_id=log_id))
...
def admin_reset_website_settings():
    ...
    try:
        ...
        app.logger.info(f"Website settings cleared by admin {email_id}")
        
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error clearing website settings: {str(e)}")
...
# ========================
# INSERT THESE ROUTES AFTER LINE 5567 in app2.py (after admin_reset_website_settings function)

@app.route('/admin/blog_categories')
@admin_required
@csrf_exempt
def admin_blog_categories():
    ...
    try:
        ...
        app.logger.error(f"Error loading blog categories: {str(e)}")
        flash('Error loading blog categories', 'danger')
        return redirect(url_for('admin_dashboard'))

@app.route('/admin/blog_category/add', methods=['GET', 'POST'])
@admin_required
@csrf_exempt
def admin_add_blog_category():
    ...
    app.logger.info(f"admin_add_blog_category called - Method: {request.method}")

    if request.method == 'POST':
        try:
            app.logger.info(f"POST request received for add blog category")
            app.logger.info(f"Form data: {request.form}")
            app.logger.info(f"Admin session: admin_id={session.get('admin_id')}, email={session.get('email_id')}")
            ...
            if not name:
                app.logger.warning("Category name is empty")
                flash('Category name is required', 'danger')
                return redirect(url_for('admin_add_blog_category'))

            # Check if category already exists
            existing = BlogCategory.query.filter_by(name=name).first()
            if existing:
                app.logger.warning(f"Category '{name}' already exists")
            ...
            app.logger.info(f"Blog category '{name}' added successfully")
            flash('Blog category added successfully!', 'success')
            return redirect(url_for('admin_blog_categories'))

        except Exception as e:
            db.session.rollback()
            app.logger.error(f"Error adding blog category: {str(e)}")
            app.logger.error(f"Traceback: {traceback.format_exc()}")
...
@app.route('/admin/blog_category/edit/<int:id>', methods=['GET', 'POST'])
@admin_required
@csrf_exempt
def admin_edit_blog_category(id):
    ...
    if request.method == 'POST':
        try:
            ...
            if not name:
                flash('Category name is required', 'danger')
                return redirect(url_for('admin_edit_blog_category', id=id))
            ...
            db.session.rollback()
            app.logger.error(f"Error updating blog category: {str(e)}")
            flash(f'Error updating category: {str(e)}', 'danger')
            return redirect(url_for('admin_edit_blog_category', id=id))

    return render_template('admin/edit_blog_category.html', category=category)

@app.route('/admin/blog_category/delete/<int:id>', methods=['POST'])
@admin_required
@csrf_exempt
def admin_delete_blog_category(id):
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error(f"Error deleting blog category: {str(e)}")
...
def admin_blog_upload_image():
    ...
    try:
        app.logger.info("Image upload request received")

        # Check admin authentication manually (for JSON response)
        if 'admin_id' not in session:
            app.logger.error("Unauthorized upload attempt - no admin_id in session")
            return jsonify({'error': {'message': 'Unauthorized. Please log in as admin.'}}), 401

        # Check permission
        email_id = session.get('email_id')
        if not Admin.check_permission(email_id, 'blog_management'):
            app.logger.error("Unauthorized upload attempt - no blog_management permission")
            return jsonify({'error': {'message': 'You do not have permission to upload blog images.'}}), 403

        if 'upload' not in request.files:
            app.logger.error("No 'upload' field in request.files")
            return jsonify({'error': {'message': 'No file uploaded'}}), 400

        file = request.files['upload']

        if not file or not file.filename:
            app.logger.error("File object is empty or has no filename")
        ...
        app.logger.info(f"Uploading file: {filename}, extension: {file_ext}")
        ...
        upload_folder = os.path.join(app.root_path, 'static', 'uploads', 'blogs', 'content')
        os.makedirs(upload_folder, exist_ok=True)
    ...
    if request.method == 'POST':
        try:
            ...
            for q, a in zip(faq_questions, faq_answers):
                if q.strip() and a.strip():  # Only add if both question and answer exist
                    faqs.append({
                        'question': q.strip(),
                        'answer': a.strip()
                    })
            ...
            if 'image' in request.files:
                ...
                if file and file.filename:
                    # Create uploads directory if it doesn't exist
                    upload_folder = os.path.join(app.root_path, 'static', 'uploads', 'blogs')
                    os.makedirs(upload_folder, exist_ok=True)
            ...
            db.session.rollback()
            app.logger.error(f"Error adding blog: {str(e)}")
...
def admin_edit_blog(id):
    ...
    if request.method == 'POST':
        try:
            ...
            if not title:
                flash('Blog title is required', 'danger')
                return redirect(url_for('admin_edit_blog', id=id))
            ...
            if 'image' in request.files:
                ...
                if file and file.filename:
                    ...
                    if blog.image:
                        old_image_path = os.path.join(app.root_path, 'static', 'uploads', 'blogs', blog.image)
...
def admin_delete_blog(id):
    ...
    try:
        ...
        if blog.image:
            image_path = os.path.join(app.root_path, 'static', 'uploads', 'blogs', blog.image)
...
@app.route('/admin/webstories', methods=['GET'])
@admin_required
@csrf_exempt
...
@app.route('/admin/webstory/upload-image', methods=['POST'])
@admin_required
@csrf.exempt
def admin_webstory_upload_image():
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            upload_dir = os.path.join(app.root_path, 'static', 'uploads', 'webstories', 'slides')
        ...
        app.logger.error(f"Error uploading webstory image: {str(e)}")
        return jsonify({
            'uploaded': False,
            'error': {'message': str(e)}
        })

@app.route('/admin/webstory/check-slug', methods=['POST'])
@admin_required
def admin_webstory_check_slug():
    ...
    try:
        ...
        app.logger.error(f"Error checking slug: {str(e)}")
        return jsonify({'available': False, 'message': 'Error checking slug'}), 500

@app.route('/admin/webstory/add', methods=['GET', 'POST'])
@admin_required
@csrf_exempt
    ...
    if request.method == 'POST':
        try:
            ...
            if 'cover_image' in request.files:
                ...
                if file and file.filename != '' and allowed_file(file.filename):
                    filename = str(uuid.uuid4()) + '_' + secure_filename(file.filename)
                    upload_dir = os.path.join(app.root_path, 'static', 'uploads', 'webstories')
                    os.makedirs(upload_dir, exist_ok=True)
            ...
            return redirect(url_for('admin_webstories'))

        except Exception as e:
            db.session.rollback()
            app.logger.error(f"Error creating webstory: {str(e)}")
...
@app.route('/admin/webstory/edit/<int:id>', methods=['GET', 'POST'])
@admin_required
@csrf_exempt
def admin_edit_webstory(id):
    ...
    if request.method == 'POST':
        try:
            ...
            if 'cover_image' in request.files:
                ...
                if file and file.filename != '' and allowed_file(file.filename):
                    ...
                    if webstory.cover_image:
                        old_image_path = os.path.join(app.root_path, 'static', 'uploads', 'webstories', webstory.cover_image)
                        if os.path.exists(old_image_path):
                            try:
                                os.remove(old_image_path)
                            except:
            ...
            db.session.rollback()
            app.logger.error(f"Error updating webstory: {str(e)}")
            flash(f'Error updating webstory: {str(e)}', 'danger')

    return render_template('admin/edit_webstory.html', webstory=webstory)

@app.route('/admin/webstory/delete/<int:id>', methods=['POST'])
@admin_required
@csrf_exempt
def admin_delete_webstory(id):
    ...
    try:
        ...
        if webstory.cover_image:
            image_path = os.path.join(app.root_path, 'static', 'uploads', 'webstories', webstory.cover_image)
            if os.path.exists(image_path):
                try:
                    os.remove(image_path)
                except:
        ...
        if webstory.slides:
            for slide in webstory.slides:
                if slide.get('image'):
                    ...
                    slide_image_path = os.path.join(app.root_path, 'static', 'uploads', 'webstories', 'slides', slide_image)
        ...
        db.session.rollback()
        app.logger.error(f"Error deleting webstory: {str(e)}")
...
def subscribe(plan_id):
    ...
    try:
        ...
        # Payment record will be created only when user submits checkout form
...
def generate_invoice_pdf(payment):
    ...
    # Use app.root_path to ensure correct path resolution on server
    logo_path = os.path.join(app.root_path, 'assert', '4d-logo.webp')
...
def download_invoice(payment_id):
    try:
        ...
        if pdf_buffer is None:
            app.logger.error(f"generate_invoice_pdf returned None for payment {payment_id}")
        ...
        app.logger.error(f"Error generating invoice for payment {payment_id}: {str(e)}")
        app.logger.error(traceback.format_exc())
        flash('Error generating invoice. Please try again later.', 'error')
        return redirect(url_for('user_subscriptions'))
...
def generate_invoice_pdf_for_email(payment, subscription, user):
    ...
    try:
        ...
        app.logger.error(f"Error generating invoice PDF: {str(e)}")
        return None
...
"""
        ...
        if invoice_pdf:
            ...
            app.logger.info(f"Invoice PDF attached to payment confirmation email")
        ...
        app.logger.info(f"Payment confirmation email sent to {user.company_email}")
    ...
    try:
        ...
        if not sub:
            ...
            if user and user.trial_tokens >= tokens_needed:
                ...
                db.session.commit()
                app.logger.info(f"User {user_id}: Used {tokens_needed} trial token(s). Remaining: {user.trial_tokens}")
                return {
                    'success': True,
                ...
                return {
                    ...
                    'usage_breakdown': f'Need {tokens_needed} tokens but only {user.trial_tokens} trial tokens remaining',
                    'trial_tokens_remaining': user.trial_tokens
                }
            else:
                return {
...
def record_usage_log(user_id, subscription_id, operation_type, details=None, tokens_used=1, is_trial=False):
    """
...
def subscription_required_with_tokens(tokens=1):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # Try to increment usage (this handles subscription tokens AND trial tokens)
            ...
            if not usage_result['success']:
                if usage_result['reason'] == 'no_tokens':
                    ...
                    flash(f"Not enough trial tokens. You have {usage_result.get('trial_tokens_remaining', 0)} remaining but need {tokens}.", "warning")
                    return redirect(url_for('user_subscriptions'))
            ...
            if usage_result.get('is_trial'):
                # Trial token usage - record usage log and show notification
                ...
                flash(f"Used {tokens_used} trial token{'s' if tokens_used != 1 else ''}. {remaining} trial token{'s' if remaining != 1 else ''} remaining.", "info")
                app.logger.info(f"Trial tokens used by user {user_id}: {tokens_used}. Remaining: {remaining}")
            else:
                ...
                if additional_tokens_used > 0:
                    # Used additional tokens (daily limit reached)
                    flash(f"Used {tokens} token{'s' if tokens != 1 else ''} ({tokens_from_daily} daily + {additional_tokens_used} additional). Daily limit reached.", "info")
                else:
                    # Used only daily quota
                    flash(f"Used {tokens} token{'s' if tokens != 1 else ''}. {daily_remaining} daily token{'s' if daily_remaining != 1 else ''} remaining.", "info")

                app.logger.info(f"Subscribed user {user_id}: Used {tokens} tokens. Daily: {new_daily_usage}/{daily_limit}, Additional used: {additional_tokens_used}")
...
def subscription_check_only(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not current_user.is_authenticated:
            if 'user_id' not in session:
                app.logger.warning(f"subscription_check_only: User not authenticated, no session")
                flash("Please login to access this feature.", "warning")
                return redirect(url_for('login'))
            user_id = session.get('user_id')
        else:
        ...
        app.logger.info(f"subscription_check_only: Checking subscription for user {user_id}")
        ...
        if not active_subscription:
            ...
            if user and user.trial_tokens > 0:
                app.logger.info(f"subscription_check_only: No subscription but user {user_id} has {user.trial_tokens} trial tokens, allowing access")
                return f(*args, **kwargs)

            app.logger.warning(f"subscription_check_only: No active subscription and no trial tokens for user {user_id}")
            flash("Please subscribe to access this feature.", "warning")
            return redirect(url_for('user_subscriptions'))

        app.logger.info(f"subscription_check_only: Active subscription found for user {user_id}, allowing access to {f.__name__}")
...
def index():
        ...
        if True:  # Always show stats for logged-in users
            ...
            if active_subscription:
                ...
                for log in today_trial_logs:
                    try:
                        ...
                        app.logger.error(f"Error parsing trial token usage from log {log.id}: {str(e)}")
                        today_token_usage += 1
            ...
            for log in yesterday_logs:
                try:
                    ...
                    app.logger.error(f"Error parsing yesterday token usage from log {log.id}: {str(e)}")
                    yesterday_token_usage += 1
            ...
            for log in all_logs:
                try:
                    ...
                    app.logger.error(f"Error parsing total token usage from log {log.id}: {str(e)}")
                    total_token_usage += 1

            # For users who used trial tokens BEFORE usage logging was implemented,
            # add any unaccounted trial tokens to the total
            # Check if there are any trial usage logs - if not, add trial tokens used
            ...
            if trial_logs_count == 0:
                ...
                trial_tokens_used = 5 - trial_tokens
                if trial_tokens_used > 0:
                    total_token_usage += trial_tokens_used
    ...
    return render_template('index.html',
                      ...
                      trial_tokens_used=trial_tokens_used,  # Trial tokens used count
...
def keyword_search():
    ...
    if request.method == 'GET':
        ...
        if is_explicit_refresh:
            clear_search_results('keyword_search')
            app.logger.info("Cleared keyword_search results - explicit refresh")
    ...
    if request.method == 'POST':
        ...
        if url_input:
            try:
                app.logger.info(f"📝 Keyword search POST: analyzing {url_input}")
                home_links, other_links, robots_info = analyze_links(
                    url=url_input,
                    respect_robots=respect_robots
                )

                app.logger.info(f"📊 Analysis complete: {len(home_links)} home links, {len(other_links)} other links")
                ...
                if not store_result:
                    app.logger.error("❌ Failed to store search results")
                ...
                app.logger.info(f"✅ Returning results immediately without redirect")

            except Exception as e:
                app.logger.error(f"❌ Error in keyword_search: {str(e)}", exc_info=True)
                flash(f"Error analyzing URL: {str(e)}", "danger")
                return redirect(url_for('keyword_search'))
    else:
        ...
        if links_data:
            app.logger.info(f"✅ Retrieved cached results for {url_input}")
        else:
            app.logger.info("ℹ️  No cached results found")
...
@app.route('/keyword_detail_ajax', methods=['POST'])
@login_required
...
def keyword_detail():
    ...
    if not home_links:
        try:
            app.logger.info(f"No cached links found, analyzing {link} for related links")
            home_links_new, other_links, robots_info = analyze_links(
                url=link,
                respect_robots=True
            )
            home_links = home_links_new
            app.logger.info(f"Found {len(home_links)} related links")
        except Exception as e:
            app.logger.error(f"Error analyzing links: {str(e)}")
            home_links = []
    ...
    try:
        ...
        if extracted_text and not extracted_text.startswith("Error"):
            ...
            app.logger.info(f"Extracting keywords from displayed text of length: {len(extracted_text)}")
            app.logger.info(f"Using weighted text of length: {len(weighted_text)} for keyword extraction")
            ...
            app.logger.info(f"Top 10 keywords extracted: {len(auto_keywords)}")
        else:
            app.logger.warning(f"Text extraction failed or returned error: {extracted_text[:100] if extracted_text else 'None'}")
...
# Helper function to highlight keywords for AJAX response
...
def image_search():
    ...
    if request.method == 'GET':
        ...
        if is_explicit_refresh or is_from_other_page:
            clear_search_results('image_search')
            app.logger.info("Cleared image_search results - fresh navigation or explicit refresh")
    ...
    if request.method == 'POST':
        ...
        if url_input:
            try:
                app.logger.info(f"📝 Image search POST: analyzing {url_input}")
                home_links, other_links, robots_info = analyze_links(
                    url=url_input,
                    respect_robots=respect_robots
                )
                ...
                app.logger.error(f"❌ Error in image_search: {str(e)}", exc_info=True)
        ...
        if not is_from_other_page:
            ...
            app.logger.info("ℹ️  Fresh page load - not loading cached results")
...
def image_detail():
    ...
    app.logger.info("=" * 80)
    app.logger.info("IMAGE_DETAIL ROUTE ACCESSED")
    app.logger.info(f"Request args: {request.args}")
    app.logger.info(f"Request method: {request.method}")
    app.logger.info(f"Request path: {request.path}")

    link = request.args.get('link')
    app.logger.info(f"Link parameter: {link}")

    if not link:
        app.logger.error("NO LINK PROVIDED!")
    ...
    if images is None:
        ...
        app.logger.info(f"User {user_id} requesting image analysis for {link} (not in cache)")

        try:
            app.logger.info(f"Extracting images from {link}")
            images = extract_images(link)
            # Ensure images is always a list
            if images is None:
                images = []

            app.logger.info(f"Successfully extracted {len(images)} images from {link}")
            ...
            app.logger.info(f"Processed image data, setting cache")
            cache.set(cache_key, images, timeout=3600)  # Cache for 1 hour

        except Exception as e:
            app.logger.error(f"Error extracting images from {link}: {str(e)}", exc_info=True)
            flash(f"Error extracting images: {str(e)}", "error")
            return redirect(url_for('image_search'))
    else:
        # Images loaded from cache
        app.logger.info(f"User {user_id} viewing cached image results for {link} ({len(images)} images)")

    # Always record search history since tokens are consumed by decorator
    # This ensures the URL shows in recent analysis regardless of cache
    if images:
        add_search_history(user_id, "Image Analysis", link)

    app.logger.info(f"=" * 80)
    app.logger.info(f"RENDERING image_detail.html with {len(images)} images")
    app.logger.info(f"=" * 80)

    try:
        response = render_template('image_detail.html', link=link, images=images)
        app.logger.info("Template rendered successfully, returning response")
        return response
    except Exception as e:
...
def download_image_csv():
    ...
    try:
        ...
        app.logger.error(f'Error generating CSV file: {str(e)}')
        flash(f"Error generating CSV file: {str(e)}", "error")
        return redirect(url_for('image_search'))

# FIXED DOWNLOAD ROUTES FOR IMAGE EXTRACTION TOOL
# Replace these routes in your app.py file starting from line 9740

@app.route('/download_single_image', methods=['POST'])
@login_required
def download_single_image():
    ...
    try:
        ...
        app.logger.info(f'Attempting to download: {image_url}')
        ...
        headers = {
            ...
            'Referer': base_url + '/',  # Use base URL as referer
            'Origin': base_url,
        ...
        # Try with SSL verification first, fall back to no verification if SSL errors occur
        try:
            response = requests.get(
                image_url,
                headers=headers,
                timeout=30,
                verify=ssl_verify,  # Use certifi certificates for better SSL compatibility
            ...
            app.logger.warning(f'SSL verification failed, retrying without verification: {ssl_err}')
            response = requests.get(
                image_url,
                headers=headers,
                timeout=30,
        ...
        app.logger.info(f'Downloaded content: {len(image_content)} bytes, Content-Type: {content_type}')

        # Debug: Log first 16 bytes of content
        if len(image_content) >= 16:
            hex_bytes = ' '.join(f'{b:02X}' for b in image_content[:16])
            app.logger.info(f'First 16 bytes (hex): {hex_bytes}')
        ...
        def detect_format_from_magic_bytes(content):
            ...
            try:
                ...
                if '<svg' in text_start or ('<?xml' in text_start and 'svg' in text_start):
                    app.logger.info('✓ SVG signature detected!')
                    return '.svg', 'image/svg+xml'
            except:
                pass

            app.logger.warning('✗ No magic bytes signature matched')
            return None, None
        ...
        if file_ext and content_type_detected:
            app.logger.info(f'✓ Format detected from MAGIC BYTES: {file_ext} ({content_type_detected})')
                ...
                try:
                    ...
                    if detected_type:
                        ...
                        if detected_type in type_mapping:
                            ...
                            app.logger.info(f'Detected format from imghdr: {detected_type} -> {file_ext}')
                except Exception as e:
                    app.logger.warning(f'imghdr detection failed: {e}')
        ...
        if not file_ext:
            ...
            if response_content_type and response_content_type.startswith('image/'):
                ...
                if response_content_type in content_type_mapping:
                    ...
                    app.logger.info(f'Detected from Content-Type header: {response_content_type} -> {file_ext}')
        ...
        if not file_ext:
            ...
            if url_ext and len(url_ext) >= 2 and url_ext.startswith('.'):
                ...
                app.logger.info(f'Detected extension from URL: {file_ext}')

        # FINAL VALIDATION: Check if we detected a valid image format
        if not file_ext or len(file_ext) < 2:
            # No format detected - this is likely not an image
        ...
        app.logger.info(f'Created temp file: {temp_file_path}, Filename: {filename}, Content-Type: {content_type}, Size: {len(image_content)} bytes')
        ...
        def cleanup(response):
            try:
                if temp_file_path and os.path.exists(temp_file_path):
                    ...
                    app.logger.info(f'Cleaned up temp file: {temp_file_path}')
            except Exception as e:
        ...
        app.logger.error(f'Error downloading image from {image_url}: {str(e)}')
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                os.unlink(temp_file_path)
            except:
        ...
        app.logger.error(f'Unexpected error in download_single_image: {str(e)}')
        app.logger.error(traceback.format_exc())
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                os.unlink(temp_file_path)
...
def download_images_zip():
    ...
    try:
        ...
        app.logger.info(f'Creating ZIP file with {len(images)} images')
        ...
        app.logger.info(f'Creating ZIP at: {temp_zip_path}')
        ...
        with zipfile.ZipFile(temp_zip_path, 'w', zipfile.ZIP_DEFLATED, allowZip64=True) as zip_file:
            ...
            for idx, image_data in enumerate(images, 1):
                ...
                try:
                    app.logger.info(f'Downloading image {idx}/{len(images)}: {image_url}')
                    ...
                    try:
                        ...
                        # SSL verification failed, retry without verification
                        app.logger.warning(f'SSL verification failed for {image_url}, retrying: {ssl_err}')
                        response = requests.get(
                            image_url,
                            headers=headers,
                            timeout=30,
                    ...
                    app.logger.info(f'Downloaded image {idx}: {len(image_content)} bytes')

                    # Detect actual image format using multiple methods
                    file_ext = detect_format_from_magic_bytes(image_content)

                    if file_ext:
                        app.logger.info(f'Image {idx} detected format from magic bytes: {file_ext}')
                    ...
                    if not file_ext and imghdr is not None:
                        try:
                            ...
                            if detected_type:
                                ...
                                app.logger.info(f'Image {idx} detected format from imghdr: {detected_type} -> {file_ext}')
                        except Exception as e:
                            app.logger.warning(f'Image {idx} imghdr detection failed: {e}')
                    ...
                    if not file_ext:
                        ...
                        if content_type and content_type.startswith('image/'):
                            ...
                            file_ext = content_type_to_ext.get(content_type)
                            if file_ext:
                                app.logger.info(f'Image {idx} detected extension from Content-Type: {content_type} -> {file_ext}')
                    ...
                    if not file_ext:
                        ...
                        if url_ext and len(url_ext) >= 2 and url_ext.startswith('.'):
                            file_ext = url_ext
                            app.logger.info(f'Image {idx} detected extension from URL: {file_ext}')
                    ...
                    app.logger.info(f'Successfully added to ZIP: {filename} ({len(image_content)} bytes)')
...
"""
            ...
            app.logger.info(f'Download summary: {successful_downloads} successful, {failed_downloads} failed')
        ...
        app.logger.info(f'ZIP file created successfully: {temp_zip_path}, Size: {zip_size} bytes')

        if zip_size == 0:
            raise ValueError('ZIP file is empty - no content was added')

        if zip_size < 100:  # Suspiciously small
            app.logger.warning(f'ZIP file is very small ({zip_size} bytes), might be corrupted')
        ...
        # This allows the temp file to be deleted immediately after reading
        ...
        try:
            ...
            app.logger.info(f'Cleaned up temp ZIP file: {temp_zip_path}')
        except Exception as e:
            app.logger.error(f'Error cleaning up temp ZIP file: {e}')
            ...
            def delayed_cleanup():
                ...
                try:
                    if os.path.exists(temp_zip_path):
                        os.unlink(temp_zip_path)
                        app.logger.info(f'Delayed cleanup succeeded for: {temp_zip_path}')
                except Exception as cleanup_err:
                    app.logger.warning(f'Delayed cleanup also failed: {cleanup_err}')
        ...
        if temp_zip_path and os.path.exists(temp_zip_path):
            try:
                os.unlink(temp_zip_path)
                app.logger.info(f'Cleaned up temp ZIP file after error: {temp_zip_path}')
            except Exception as cleanup_error:
                app.logger.error(f'Error cleaning up temp ZIP file: {cleanup_error}')
...
def h_search():
    ...
    if request.method == 'POST':
        ...
        if url_input:
            try:
                app.logger.info(f"📝 Heading search POST: analyzing {url_input}")
                home_links, other_links, robots_info = analyze_links(
                    url=url_input,
                    respect_robots=respect_robots
                )
        ...
        if links_data:
            app.logger.info(f"📊 Retrieved h_search results from cache")
...
def h_detail():
    ...
    if not home_links:
        try:
            app.logger.info(f"No cached links found, analyzing {url_input} for related links")
            home_links_new, other_links, robots_info = analyze_links(
                url=url_input,
                respect_robots=True
            )
...
def meta_search():
    ...
    if request.method == 'POST':
        ...
        if url_input:
            try:
                app.logger.info(f"📝 Meta search POST: analyzing {url_input}")
                home_links, other_links, robots_info = analyze_links(
                    url=url_input,
                    respect_robots=respect_robots
                )
        ...
        if links_data:
            app.logger.info(f"📊 Retrieved meta_search results from cache")
...
def site_structure():
    ...
    if request.method == "POST":
        ...
        # Debug logging
        app.logger.info(f"SESSION BEFORE SAVE: job_id={session.get('job_id')}, user_id={session.get('user_id')}")
        ...
        app.logger.info(f"Created crawl job {job_id} for URL: {start_url}")
        ...
        try:
            ...
            app.logger.info(f"Crawler submitted to executor for job {job_id}")
        except Exception as e:
            app.logger.error(f"Error submitting crawler: {e}")
        ...
        app.logger.info(f"Redirecting to loading page for job {job_id}")
...
def run_async_in_thread_with_progress(coro, job_id):
    ...
    try:
        app.logger.info(f"Background thread started for job {job_id}")
        ...
        if job_id in crawl_status:
            ...
            if status_data and status_data.get('status') != 'completed':
                app.logger.warning(f"Job {job_id} finished but status is {status_data.get('status')}, forcing completion")
                status_data['status'] = 'completed'
                status_data['progress'] = 100
                crawl_status[job_id] = status_data  # Save back to Redis
                app.logger.info(f"Forced completion for job {job_id}, status saved to Redis")
            else:
                app.logger.info(f"Job {job_id} already marked as completed")
        else:
            app.logger.error(f"Job {job_id} not found in crawl_status after completion")

        app.logger.info(f"Background thread completed for job {job_id}")
        return result

    except Exception as e:
        app.logger.error(f"Error in background task for job {job_id}: {str(e)}")
        if job_id in crawl_status:
            status_data = crawl_status.get(job_id)
            if status_data:
...
def loading():
    ...
    # Debug logging
    app.logger.info(f"SESSION ON LOADING: job_id={job_id}, session_keys={list(session.keys())}")

    # If not in session, try backup cookie
    if not job_id:
        job_id = request.cookies.get('job_id_backup')
        app.logger.info(f"Trying backup cookie: job_id_backup={job_id}")

        # If found in cookie, restore to session
        if job_id:
            session['job_id'] = job_id
            session.modified = True
            app.logger.info(f"Restored job_id from backup cookie: {job_id}")

    if not job_id or job_id not in crawl_status:
        app.logger.warning(f"Loading page accessed without valid job_id. job_id={job_id}, crawl_status_keys={list(crawl_status.keys())[:5]}")
        flash("No active crawl job found. Please start a new crawl.", "warning")
        return redirect(url_for("site_structure"))

    app.logger.info(f"Loading page accessed for job_id: {job_id}, status: {crawl_status[job_id]['status']}")
...
def progress(job_id):
    ...
    # The crawler now updates progress in real-time via callback
    return jsonify({
...
async def main_crawl(start_url, job_id):
    ...
    try:
        app.logger.info(f"Starting crawl for job {job_id}")
        ...
        if status_data:
            ...
            crawl_status[job_id] = status_data  # Save back to Redis
            app.logger.info(f"Updated job {job_id} status to running, progress 10%")
        ...
        async def update_progress(progress_pct):
            ...
            if status_data:
                ...
                crawl_status[job_id] = status_data  # Save back to Redis
                app.logger.info(f"Progress update for job {job_id}: {progress_pct}%")
        ...
        app.logger.info(f"Crawl completed for job {job_id}. Processing {len(url_status)} URLs")
        ...
        if status_data:
            ...
            crawl_status[job_id] = status_data  # Save back to Redis
            app.logger.info(f"Updated job {job_id} progress to 90%")

        # Save results
        save_to_json(url_status, home_links, other_links, job_id)

        app.logger.info(f"Results saved for job {job_id}")
        ...
        if status_data:
            ...
            status_data['progress'] = 100
            crawl_status[job_id] = status_data  # Save back to Redis
            app.logger.info(f"Job {job_id} marked as completed with progress 100%")

        return True

    except Exception as e:
        app.logger.error(f"Error in main_crawl for job {job_id}: {str(e)}")
        status_data = crawl_status.get(job_id)
        if status_data:
            status_data['status'] = 'failed'
            status_data['progress'] = 0
        ...
        try:
            ...
            if secret_key:
                if not recaptcha_response:
                    app.logger.warning("reCAPTCHA validation failed: No response provided")
                    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                try:
                    ...
                    if not verify_result.get('success', False):
                        app.logger.warning(f"reCAPTCHA validation failed for {email}")
                        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
...
def pricing():
    ...
    # Get active subscription plans ordered by tier
...
def cleanup_crawl_status_memory():
    """
    Clean up completed/failed crawl jobs from Redis that are older than 1 hour
    This prevents memory buildup in Redis
    """
    try:
        # Use the manager's built-in cleanup method
        cleaned_count = crawl_status_manager.cleanup_old_jobs(max_age_seconds=3600)  # 1 hour

        if cleaned_count > 0:
            app.logger.info(f"✅ Cleaned up {cleaned_count} old crawl status entries from Redis")

        return cleaned_count

    except Exception as e:
        app.logger.error(f"❌ Error cleaning up crawl status: {str(e)}")
        return 0

# Note: setup_daily_cleanup_scheduler function is defined earlier in the file (around line 2107)
# with subscription expiry check included. The duplicate has been removed.
...
# REMOVED DUPLICATE SESSION-BASED FUNCTIONS - Using cache-based ones below
...
def safe_analyze_links(url, respect_robots=True, timeout=30):
    ...
    try:
        ...
        home_links, other_links, robots_info = analyze_links(
            ...
            respect_robots=respect_robots,
            retry_count=3,
            delay_between_retries=3,  # Increased from 2
            headless=True
        )
...
def store_search_results(search_type, url, home_links, other_links, robots_info):
    ...
    try:
        user_id = session.get('user_id')
        if not user_id:
            app.logger.error(f"store_search_results: No user_id in session!")
            return False
        ...
        app.logger.info(f"✅ STORED in cache: {cache_key} | URL: {url} | Home: {len(home_links)} | Other: {len(other_links)}")
        return True

    except Exception as e:
        app.logger.error(f"❌ Error storing search results in cache: {str(e)}", exc_info=True)
        return False
...
def get_search_results(search_type):
    ...
    try:
        ...
        app.logger.info(f"🔍 GET_SEARCH_RESULTS: search_type={search_type}, cache_key={cache_key}")

        if not cache_key:
            app.logger.warning(f"⚠️  No cache_key found in session for {search_type}")
            return '', None, None

        cache_data = cache.get(cache_key)
        if not cache_data:
            # Cache expired or missing
            app.logger.warning(f"⚠️  Cache data not found for key: {cache_key}")
        ...
        app.logger.info(f"✅ RETRIEVED from cache: {cache_key} | URL: {url} | Home: {len(home_links)} | Other: {len(other_links)}")
        return url, links_data, robots_info

    except Exception as e:
        app.logger.error(f"❌ Error retrieving search results from cache: {str(e)}", exc_info=True)
        return '', None, None
...
def get_user_token_summary(user_id):
    ...
    try:
        ...
        daily_limit_reached = active_subscription.current_usage >= active_subscription.get_total_usage_limit()
...
def blog_detail(slug):
    ...
    try:
        ...
        app.logger.info(f"Blog '{blog.title}' - ORM schema_data: {blog.schema_data}")
